...
Recovery uses a whole-log validation unit. Clean EOF succeeds only when the cursor is exactly between complete frames. A non-empty partial header, body, or checksum returns `Err`; a checksum mismatch or invalid nested key/value length also returns `Err`. Stage every decoded entry separately and mutate the output skiplist only after the complete WAL file validates. Therefore, if a valid first frame is followed by a corrupt or truncated frame, recovery returns `Err` and leaves the output skiplist unchanged rather than replaying the valid prefix.

Append the complete WAL frame before inserting any entry into the memtable, and publish `latest_commit_ts` only after every memtable entry is installed. A WAL error therefore exposes no in-memory update. After a process stop, recovery may replay a completely valid persisted log or lose an unsynchronized frame, but it must never expose a prefix of the chosen whole-log recovery unit. Synchronization remains a separate durability guarantee.
...
Before approving the checkpoint, independently calculate one encoded entry, exercise all four included/excluded range combinations through the memtable and again after flush into L0 and a leveled SST, and truncate one WAL batch in its header, body, and checksum. Also corrupt the second frame after one valid frame and verify that whole-log recovery leaves the output empty. Explain which guarantee each case checks. After focused tests pass, have the agent point to the shared user-bound conversion, the internal-key comparison, and the condition that prevents an SST split within one user key.
