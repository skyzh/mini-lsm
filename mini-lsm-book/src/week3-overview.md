...
| [Day 5: Transactional Writes](./week3-05-txn-occ.md) | Transaction writes are not private or atomic. | A transaction reads its own workspace and commits one atomic WAL batch. |
...
4. For persistence changes, identify the bytes that make one batch atomic and the durable source of the recovered commit timestamp.
...
- why a WAL batch must be appended before its entries become visible in the memtable;
